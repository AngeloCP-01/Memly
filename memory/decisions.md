...
| D092 | Custom MemlyToast replaces Material 3 Snackbar | Snackbars rendered behind the bottom nav bar, making them invisible. Custom centered overlay toast with fade+scale animation, auto-dismiss after 3s. Uses `AnimatedVisibility` + `scaleIn`/`fadeIn`. Error style (errorContainer) and success style (secondaryContainer). |
| D093 | Collection detail: 2-column grid with stacked polaroid cards | List view was visually flat. Grid with polaroid-style stacked photos (random rotations, white borders, shadows) makes collection browsing more visual and engaging. |
| D094 | Memory detail: BottomSheetScaffold with full-screen carousel | Plain scrolling layout wasted screen real estate on images. BottomSheetScaffold lets media fill the screen; details are in a draggable bottom sheet. Carousel with padding/spacing shows peek of next image. |
| D095 | Mood-based color theming on detail screen | FAB, chips, tags, location pill, sheet background all derive from memory's mood color. `lerp(surface, moodColor, 0.08f)` for opaque tinted background. `luminance()` check for readable FAB icon color. |
| D096 | ContentScale.FillWidth + clip for rounded non-cropping images | `Fit` + `clip` doesn't show rounded corners (image doesn't touch clip boundary). `Crop` + `clip` works but loses content. `FillWidth` fills card width while preserving aspect ratio, so `clip(RoundedCornerShape)` visually cuts corners on the actual image. |
| D097 | Edit/Save buttons inside bottom sheet instead of FAB | FAB overlapped the sheet drag handle and was visually disconnected from the edit form. Full-width Button inside sheet content is more discoverable and scrolls naturally with edit fields. |
| D098 | Edit screen media grid matches CaptureScreen 3-column grid | Consistency between add and edit flows. Same visual language: order badges, video play icons, remove buttons with surface-alpha backgrounds. Removed media shown as "Undo remove" AssistChips instead of dimmed overlays. |
| D099 | Map and PlacePickerDialog default to Philippines (12.8797, 121.7740) | User locale preference. Both MapScreen and PlacePickerDialog use same default coordinates at zoom 6. PlacePickerDialog still auto-detects current location on open if permission granted. |
| D100 | Custom zoom buttons replace osmdroid built-in zoom controls in PlacePickerDialog | Built-in zoom buttons at bottom-right were covered by the confirm button. Custom SmallFABs (Zoom In / Zoom Out / My Location) stacked vertically above the confirm section. Built-in controls hidden via `zoomController.setVisibility(NEVER)`. |
| D101 | Custom awaitEachGesture for pinch-to-zoom in full-screen viewer | `detectTransformGestures` consumes all drag events including single-finger swipes, blocking HorizontalPager navigation. Custom gesture handler with `awaitEachGesture` only consumes multi-touch (pinch zoom) and single-finger drag when zoomed. At 1x zoom, single-finger events pass through to pager. |
| D102 | Transparent tap overlay on carousel videos for full-screen entry | `VideoPlayer`'s `PlayerView` consumes all touch events for playback controls, preventing the parent `clickable` modifier from receiving taps. A transparent `Box(Modifier.fillMaxSize().clickable{})` layered on top intercepts taps to open full-screen viewer. |
| D103 | Photon geocoder replaces Nominatim for place search | Nominatim has no fuzzy matching, no location bias, poor results for POIs/businesses. Photon (`photon.komoot.io`) provides fuzzy search, location-biased ranking, and better autocomplete. Nominatim kept for reverse geocoding only (Photon lacks reverse endpoint). |
| D104 | Country filter dropdown on place picker | Photon returns global results by default. Country filter uses `bbox` param (server-side bias) + `countrycode` property (client-side hard filter) to restrict results. Compact flag icon button (56dp) next to search bar. Defaults to Philippines. 13 countries + Worldwide option. |
| D105 | Debounced autocomplete (300ms) for place search | Nominatim required manual search submission. Photon's public API is generous enough for as-you-type search. 300ms debounce via coroutine Job cancellation prevents excessive requests. Triggers at 2+ characters. |
| D106 | Moved Map Enhancement and Data Management from Phase 2 to Phase 3 | Phase 2 sections 0-5 complete. Map Enhancement (Google Maps migration) and Data Management (backup/restore/export) deferred to Phase 3 as sections 7-8 to keep Phase 2 scope tight. Phase 2 now ends with Integration & Polish (Section 6) covering sections 0-5 only. |
